    ...
    do {
        ...
        @autoreleasepool {
            ...
            if ([fileManager fileExistsAtPath:fullPath] && !isDirectory && !overwrite) {
                // If file exists and overwrite is not allowed, compare CRC32 where possible and skip only if same
                uint32_t existingCRC = 0;
                int32_t crcStatus = mz_file_get_crc(fullPath.fileSystemRepresentation, &existingCRC);
                if (crcStatus == MZ_OK) {
                    if (existingCRC == fileInfo.crc) {
                        // Same file contents (CRC matched) — skip extraction
                        NSLog(@"[SSZipArchive] Skipping extraction of %@ (CRC match)", fullPath);
                        unzCloseCurrentFile(zip);
                        ret = unzGoToNextFile(zip);
                        continue;
                    } else {
                        // Different file contents — do not overwrite, so skip and warn
                        NSLog(@"[SSZipArchive] Existing file %@ differs (CRC mismatch); skipping because overwrite is NO", fullPath);
                        unzCloseCurrentFile(zip);
                        ret = unzGoToNextFile(zip);
                        continue;
                    }
                } else {
                    // Could not compute CRC; fall back to previous behavior and skip
                    unzCloseCurrentFile(zip);
                    ret = unzGoToNextFile(zip);
                    continue;
                }
